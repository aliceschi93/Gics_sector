...
```{r}
# counting how many 1 in the matrices
count <- 0
for (i in 1 : 55){
  for (j in 1 : 55){
    if (adj_matr1[i,j] ==1){
...
As we can see from the plot with $\epsilon = 0.35$ some clusters are shown as well as some isolated nodes. In particular Energy, Financial, Industrial and Utilities cluster together so we can deduce that the stocks inside these sectors follow the same trend in changing price.
...
for (firm2 in colnames(X_hat)) try({
